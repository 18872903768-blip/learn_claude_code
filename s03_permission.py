...
PERMISSION_RULES=[
    {
        "tools_name":["write_file","edit_file"],
        #args.get("path","")获取args里名为"path"键的值，如果没有这个键，返回""，防止程序崩溃
        "check":lambda args:not (WORKDIR/args.get("path","")).resolve().is_relative_to(WORKDIR),
        "message":"对超出当前目录的文件进行修改！"
     },
    {
        "tools_name":["bash"],
        "check":lambda args:any(kw in args.get("command") for kw in ["rm ","> /etc/","chmod 777","del ","rd "]),
        "message":"潜在的破坏性指令!"
    }
]
def check_rules(tools_name:str,args:dict)->str|None:
    #读取指令
    for match in PERMISSION_RULES:
        # 匹配权限规则
        if tools_name in match.get("tools_name") and match["check"](args):
            # 处理
            return match["message"]

    return None
#大门3匹配到的指令交给用户来决策
def ask_user(tools_name:str,args:dict,reason:str)->str:
    print(f"\n\033[33m⚠ {reason}\033[0m")
    print(f"TOOL:{tools_name}({args})")
...
def check_permission(block)->bool:
    #1.禁止指令
    if block.name == "bash":
        reason=deny_list(block.input.get("command"))
        if reason:
    ...
    if reason:
        result=ask_user(block.name,block.input,reason)
        if result=="deny":
            return False
...
def agent_loop(messages:list):
    while True:
        ...
        for block in message:
            ...
            allowed=check_permission(block)
            print(f"{allowed}")
            if not allowed:
                results.append({
                    "type":"tool_result",
...
if __name__ == '__main__':
    #四.搭建服务端
    #1.欢迎语
    print("s03: Permission— 在 s02 基础上加了权限认证")
    ...
    while True:
        try:
            #用户输入
            query = input("\033[037ms03 >> \033[0m")
