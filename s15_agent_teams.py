...
def claim_task(task_id:str,owner:str="agent")->str:
    ...
    if not can_start(task_id):
        deps=[p for p in task.blockedBy
              if not _task_path(p).exists() or _load_task(p).status!="completed"]
...
class MessageBus:
    ...
    def send(self,from_agent:str,to_agent:str,content:str,msg_type:str="message"):
        #创建消息
        msg = {"from_gent":from_agent,"to_agent":to_agent,"content":content,"msg_type":msg_type,"ts":time.time()}
...
def spawn_teammate_thread(name:str,role:str,prompt:str)->str:
    ...
    def run():
        ...
        sub_handlers = {
            "bash": run_bash, "read_file": run_read, "write_file": run_write,"send_message":lambda to,content:(BUS.send(name,to,content),"send")[1]}
        #开启llm循环
        for _ in range(10):
            ...
            for block in response.content:
                ...
                if block.type=="tool_use":
                    ...
                    results.append({"role":"tool_result","content":str(output),"tool_use_id":block.id})
...
def run_check_inbox()->str:
    ...
    lines=[]
    for msg in msgs:
        lines.append(f"[{msg['from_agent']}] {msg['content'][:200]}")
    return "\n".join(lines)
        ...
        # ─────────────────────────────
            ...
            if inbox:
                ...
                for message in inbox:
                    ...
                    mgs_type = message.get("msg_type","message")
