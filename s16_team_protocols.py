...
def spawn_teammate_thread(name:str,role:str,prompt:str)->str:
    ...
    def run():
        ...
        while not shutdown_requested:
            ...
            #判断name的文件是否存在
            if notification:
                inbox_json = json.dumps(notification)
                messages.append({"role":"user","content":"<inbox>" +inbox_json+"</inbox>"})
            try:
                #向llm发送信息
                response=client.messages.create(
                    model=MODEL,
            ...
            if response.stop_reason!="tool_use":
                ...
                protocol_woke_agent = False
                #为了使得teammates可以长期存在，长期工作，这里不结束而是轮询inbox
                while not shutdown_requested:
                    time.sleep(1)
                    inbox = BUS.read_inbox(name)
                    if not inbox:
                    ...
                    for msg in inbox:
                        if msg.get("type") in ("shutdown_request","plan_approval_response"):
                            ...
                            if should_stop:
                                shutdown_requested = True
                                break
                            # plan 审批结果已经被加入 messages，
                            # 需要唤醒 teammate 再调用一次 LLM
                            if msg.get("type") == "plan_approval_response":
                                protocol_woke_agent = True
                        else:
                            notification.append(msg)
                    ...
                    if notification or protocol_woke_agent:
                        break  # back to LLM turn with new messages
                # 收到 shutdown，退出 teammate 外层循环
                if shutdown_requested:
                    break
                # 收到普通消息或计划审批结果，
                # 回到 teammate 外层 while，再调用一次 LLM
                continue
            # Execute tool calls
            results=[]
            for block in response.content:
